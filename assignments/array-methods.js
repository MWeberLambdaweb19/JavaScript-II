...
let largeShirts = [];
largeShirts = runners.filter(function(largesize){
    return largeShirts.push(largesize.shirt_size === "L")
})
...
// Problem 1
// The event director, HUNGOVER on power, demands that
// you return a list of all the first names lower case... and all of the last names Upper case
let lowUp = [];
lowUp = runners.map(function(name){
    return (name.first_name.toLowerCase() + " " + name.last_name.toUpperCase() );
})
console.log(lowUp);
// Problem 2
// The event director was sacked for wasting valuable time on case sizings of names. 
// The new event director realizes that the fun run is for three different charities.
// Luckily, as people have donated a flat rate with no stipulation on where the money goes,
// the event director wants you to divvy up the money equally into three portions.
let trioMoney = []
trioMoney = runners.reduce(function(acc, currentValue){
    return  Math.round(acc + currentValue.donation/3) 
  }, 0);
console.log(trioMoney);
// Problem 3
// The event director is great at getting donations from people
// They want to harass the runners with email address to important/rich companies
// Please make a new array with that information.
let guiltTrip = []
runners.forEach (function(name) {
    guiltTrip.push (name.email + " " + name.donation + " " + name.first_name + " " + name.last_name);
})
console.log(guiltTrip);